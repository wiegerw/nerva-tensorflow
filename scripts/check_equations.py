...
LAYERS_FILE_DEFAULT = os.path.join(ROOT, 'src', 'nerva_tensorflow', 'layers.py')
...
def _normalize_tf_transpose_calls(text: str) -> str:
    # Replace tf.transpose(expr) or tr.transpose(expr) with expr.T, handling nested parentheses
        ...
        for pref in prefixes:
            ...
            while True:
                ...
                while j < len(text) and depth > 0:
                    ...
                    j += 1
                if depth != 0:
                    # unmatched, abort this occurrence
                    i = idx + 1
                    continue
                inner = text[idx + len(pref): j - 1]
                # strip outer whitespace in inner
                inner_stripped = inner.strip()
                # build replacement
                repl = f"{inner_stripped}.T"
                text = text[:idx] + repl + text[j:]
                changed = True
                # continue after replacement position
                i = idx + len(repl)
    return text
...
def norm_expr(s: str) -> str:
    ...
    s = s.replace(' .T', '.T')
    # normalize TensorFlow transpose calls (including nested cases) to .T for SymPy-style
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument('--layers-file', default=LAYERS_FILE_DEFAULT, help='Path to layers.py to inspect (default: src/nerva_tensorflow/layers.py)')
